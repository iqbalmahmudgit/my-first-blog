...
# All lines starting with from or import are lines that add some bits from other files. So instead of copying and
# pasting the same things in every file, we can include some parts with from ... import ....
...
# class Post(models.Model): – this line defines our model (it is an object). class is a special keyword that
    ...
    # def means that this is a function/method and publish is the name of the method. The naming rule is that we use
    # lowercase and underscores instead of spaces. For example, a method that calculates average price could be
    # called 'calculate_average_price'.
